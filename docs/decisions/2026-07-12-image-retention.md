...
## R4 — Wiring: one seam covers the HTML flow; JSON API wired separately
The archive is injected via FastAPI `Depends(get_upload_archive_service)` into
the JSON API extract endpoint (`app/api/highlights.py`) and into the shared
`_run_extraction` helper in `app/api/views/highlight_views.py`, which serves
both the HTML extract route and the re-extract route — so every path an image
takes through the app archives from a single seam, on success and on failure
(including model errors, via the result's `error` field). E2E server fixtures
set `UPLOADED_IMAGES_DIR` to a temp dir so test runs never write into the real
corpus.
